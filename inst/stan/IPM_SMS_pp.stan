...
transformed data {
  ...
  real mu_mu_Mmax = quantile(log(M_obs[which_M_obs] ./ A[which_M_obs]), 0.9); // prior mean of mu_Mmax
  real sigma_mu_Mmax = sd(log(M_obs[which_M_obs] ./ A[which_M_obs])); // prior SD of mu_Mmax
...
transformed parameters {
  ...
  for(i in 1:N)
  {
    ...
    M_hat[i] = A[i] * SR(SR_fun, alpha_Xbeta[i], Mmax_Xbeta[i], S[i], A[i]);
