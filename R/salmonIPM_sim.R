...
salmonIPM_sim <- function(life_cycle = "SS", SR_fun = "BH", pars, par_models = NULL, 
                          scale = TRUE, fish_data, N_age, max_age, ages = NULL)
{
  ...
  alphaRmax_MVN <- function(N_pop, sigma_alpha, sigma_Rmax, rho_alphaRmax) 
  {
  ...
  alphaRmax <- alphaRmax_MVN(N_pop, sigma_alpha, 
  ...
  K <- switch(life_cycle, 
              ...
              SMS = mu_MS*(alpha - 1)*A_pop*Rmax/alpha) 
  ...
  for(i in 1:N)
  {
    ...
    if(life_cycle=="SMS")
    {
      M_hat[i] <- SR(SR_fun, 
                     alpha[pop[i]] * exp(sum(X_alpha[i,]*beta_alpha)), 
                     Rmax[pop[i]] * exp(sum(X_Mmax[i,]*beta_Mmax)), 
                     S[i], A[i])
