...
You can verify that R knows where to find your makefile with `Sys.getenv("R_MAKEVARS_USER")`. The environment variable `R_MAKEVARS_USER` should be set to `M`, as defined above. If not, you can set it per session with `Sys.setenv()` or globally using `.Renviron`.
